...
def cnn():
    ...
    with open('Char_cnn_model_history_graph/model.json', "r") as json_file:
        loaded_model_json = json_file.read()
        model = model_from_json(loaded_model_json)
    model.load_weights("Char_cnn_model_history_graph/model_weights.h5")
    ...
    for i in range(0,100):
        y_test.append(trainy[i])
    y_test = np.array(y_test)
    y_test = np.argmax(y_test, axis=1)
    testX = np.array(testX, dtype='float32')
    print(str(trainX.shape)+" "+str(testX.shape))
    y_predicted = model.predict(testX, batch_size=128)
    print(y_predicted)
    y_predicted = np.argmax(y_predicted, axis=1)
    print(y_predicted)
    print("===================")
    print(y_test)
    nb_precision = precision_score(y_test, y_predicted,average='macro') * 100
    nb_recall = recall_score(y_test, y_predicted,average='macro') * 100
    nb_fmeasure = f1_score(y_test, y_predicted,average='macro') * 100
    nb_acc = accuracy_score(y_test,y_predicted)*100
    print(str(nb_precision)+" == "+str(nb_recall)+" "+str(nb_fmeasure)+" "+str(nb_acc))
    '''
